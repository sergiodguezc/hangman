...
export function LobbyPage({ state, messages, playerId, onLeave }: { state: PlayerGameView; messages: ChatMessage[]; playerId: string; onLeave: () => void }) {
  ...
  return <main className="lobby-page" lang={state.language}>
    <div className="lobby-layout"><section className="lobby-card">
      ...
      <div className="lobby-meta"><span>{getLanguageConfig(state.language).name}</span><span>{state.players.length} / 2 {t.players}</span></div>
    ...
    </section><RoomChat messages={messages} currentPlayerId={playerId} t={t} /></div>
  </main>
}
